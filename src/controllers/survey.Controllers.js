...
export const createSurvey = async (req,res) => {
    ...
    let {name, endDate, categories} = req.body;
...
export const updateSurveyCategories = async (req, res) =>
{
    let {id} = req.params;
    let {userId, userRole} = req.userToken;
    let {categories, action} = req.body;

    // TENES 2 ACCIONES ELIMINAR O AGREGAR
    ...
    try
    {
        ...
        if(action) //Agregar
        {
            categories.forEach( category => 
                {
                    if(!surveyById.categories.includes(category))
                    {
                        surveyById.categories.push(category);
                    }
                }
            )
        }
        else // Eliminar
        {
            categories.forEach( category => 
                {
                    surveyById.categories = surveyById.categories.filter(categoryInSurvey =>  categoryInSurvey !== category)
                }
            )
        }
...
export const updateSurveyEndDate = async (req,res) => {
    let {id} = req.params;
    let {userId, userRole} = req.userToken;
